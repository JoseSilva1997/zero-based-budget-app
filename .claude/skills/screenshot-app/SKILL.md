...
| `--window <W>x<H>` | resize the real OS window, which is the only way to reach the breakpoints in `renderer/styles/breakpoints.css`. The app scrolls inside `.main`, not the document, so no CSS-side viewport trick produces a width a media query can see. Note the window is larger than its content: `--window 1100x860` gave a 1084px viewport, and the flag prints both so you can check which side of a breakpoint you actually landed on. The app has a minimum size, so the `<1024px` tier is reachable only through the View menu's zoom. |
